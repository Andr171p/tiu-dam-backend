...
class Storage(Protocol):
    ...
    async def download_stream(
        ...
        """Потоковое скачивание файла (рекомендуется для больших файлов)"""

    async def delete(self, storage_key: str) -> None:
        """Фактическое удаление файла из хранилища"""

    async def create_presigned_upload_url(
            self, storage_key: str, content_type: str, expires_in: int = 3600
    ) -> str:
        """Генерирует подписанный URL для прямой загрузки с клиента"""
    ...
    async def get_file_info(self, storage_key: str) -> FileInfo:
